...
- Installations that use scheduled backups need either a server cron task
  pointing at `cli/backup.php --action=run` or the web-cron URL shown in the
  dashboard, called by an external scheduler.
