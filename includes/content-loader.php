...
function autoGenerateContentField($page, $fieldKey, $value) {
    ...
    $latest = null;
    $created = false;
    $ok = nibblyJsonUpdate($filepath, static function (&$data) use ($page, $fieldKey, $value, &$latest, &$created) {
        $cursor = $data;
        $exists = true;
        foreach (explode('.', $fieldKey) as $key) {
            if (!is_array($cursor) || !array_key_exists($key, $cursor)) { $exists = false; break; }
            $cursor = $cursor[$key];
        }
        if (!$exists) {
            $data['page'] = $data['page'] ?? $page;
            $data['lang'] = $data['lang'] ?? substr($page, 0, 2);
            setNestedValue($data, $fieldKey, $value);
    ...
    if ($latest !== null) { $cache = &loadContentCached($page); $cache = $latest; }
    if ($ok && $created) autoGeneratedFields($page, $fieldKey);
}
